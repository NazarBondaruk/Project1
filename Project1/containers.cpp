...
public:
	void add(string plate, desc_break add_break) {
		...
		law_breaks.push_back(add_break);
		veh_info.insert({ plate, law_breaks });
	}
	...
	void print() const {
		for (auto it : veh_info) {
			...
			for (auto viol_list : veh_info) {
				for (auto viol : viol_list.second)
					viol.print();
			}
		}
	}
