...
void carStat_network::download_data(string pathCar_fuel, string pathCar_del, string pathCar_sup) {
	...
	int read_fuelAm;
	string fuel_name, date; float fuel_am;
	auto it = carStations.begin();

	while (fin_fuel >> read_fuelAm) {
		it->total_fuel_remain = read_fuelAm;

		fin_del >> fuel_name >> date >> fuel_am;
		it->delievery.push_back(event_date(fuel_name, date, fuel_am));

		fin_sup >> fuel_name >> date >> fuel_am;
		it->suppliement.push_back(event_date(fuel_name, date, fuel_am));

		carStations.push_back(*it);
		it++;
	}
...
void truckStat_network::download_data(string pathCar_fuel, string pathCar_del, string pathCar_sup) {
	...
	int read_fuelAm;
	string fuel_name, date; float fuel_am;
	auto obj = *truckStations.begin();
	auto it = truckStations.begin();

	while (!fin_sup.eof()) {
		fin_fuel >> read_fuelAm;
		obj = *it;
		obj.total_fuel_remain = read_fuelAm;

		fin_del >> fuel_name >> date >> fuel_am;
		obj.delievery.push_back(event_date(fuel_name, date, fuel_am));

		fin_sup >> fuel_name >> date >> fuel_am;
		obj.suppliement.push_back(event_date(fuel_name, date, fuel_am));

		truckStations.push_back(obj);
		it++;
	}
