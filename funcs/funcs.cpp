...
public:
	...
	void sort(function<bool(int a, int b)> callback) {
			
			for (auto it = head; it->next != tail; it++)
				if (callback(it->data, it->next->data))
...
int main() {
	...
	cout << "Sorted list ascendinly: "; l.sort([](int a, int b) { return a > b; }); l.print();
